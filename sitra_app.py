...
- Adapte le FOND du message au TYPE d'objectif, pas seulement le ton. Un objectif d'ACQUISITION (attirer de nouveaux clients, se faire connaître, gagner des clients, élargir son audience) vise des gens qui ne connaissent pas encore la marque : donne une vraie raison de la découvrir maintenant — ce qui la différencie, pourquoi elle plutôt qu'une autre, un avantage concret à la choisir — plutôt que de simplement répéter un appel à l'achat comme si le lecteur était déjà convaincu. Un objectif de VENTE ou de LANCEMENT (annoncer un produit, écouler un stock, promouvoir une offre précise) peut en revanche pousser plus directement à l'achat immédiat. Base-toi précisément sur l'objectif donné ({objectif}) pour choisir lequel de ces angles s'applique, plutôt que de produire par défaut un post de vente générique.
...
- Interdiction des formules toutes faites et du survendu : pas de "résultat garanti", "boostez vos performances", "la solution ultime", "ne ratez pas cette opportunité", "révolutionnez votre...", ou équivalents. Ce sont des clichés creux d'IA, pas de la vente efficace.
- TON DE VENTE ASSERTIF, façon closer professionnel (style vente directe/persuasion, pas storytelling mou) : écris comme un vendeur convaincu qui assume la vente sans s'excuser, jamais timide ou neutre. Amplifie le désir avant de conclure — montre concrètement ce que le lecteur gagne à agir maintenant ou ce qu'il perd à attendre — et termine sur une conclusion qui assume l'achat plutôt que de le suggérer timidement. Ceci n'est PAS contradictoire avec l'interdiction ci-dessus : un post assertif et vendeur reste spécifique et concret (jamais un cliché générique type "ne ratez pas cette opportunité").
...
- Phrases courtes, directes, spécifiques, avec de l'énergie et de la conviction. Zéro superlatif creux, mais zéro timidité non plus.
...
st.divider()
st.markdown("""
<div style="text-align:center;color:#888;font-size:0.8rem;padding:1rem 0 0.5rem;line-height:1.8">
...
with st.expander("Confidentialité"):
    st.caption("""
NIRIKX est un projet indépendant, pas une entreprise avec un service dédié à la protection des données — voici simplement, en clair, ce que l'outil fait de vos informations :

- **Site que vous analysez** : l'URL et le contenu de la page sont envoyés à Google (API Gemini) pour générer les recommandations et le contenu IA. NIRIKX ne conserve pas le contenu du site lui-même.
- **Email** : demandé uniquement si vous activez un forfait Pro/Premium, pour associer cet accès à votre compte. Stocké dans une base de données (Neon), jamais revendu ni partagé.
- **Historique du potentiel de croissance** : les résultats sont sauvegardés (associés à l'URL analysée, pas à une personne) pour afficher votre progression dans le temps.
- **Hébergement** : Streamlit Community Cloud.

Une question sur vos données ? Écrivez à yanisaidoune1@gmail.com.
""")
